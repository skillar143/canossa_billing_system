...
<body>
    <div class="card shadow mx-auto mt-5 animated--grow-in print-card" style="width: 80%;">
        ...
        <div class="card-body p-4">
            ...
            <?php
            $discountAmount = $bill->discount ? $bill->discount->amount : 0;

            $total = $tuition + $rle + $com_fee + $spe_fee + $oth_sch_fee;
            $discount = $tuition * ($discountAmount / 100);
            ...
            <div class="col-xs-6 text-right float-end">
                <p class="m-0">Total Fee : {{ '₱' . number_format($total, 2, '.',',') }}</p>
                <p class="m-0">Discount ({{ number_format($discountAmount, 1, '.',',') }}%) : {{ '₱' . number_format($discount, 2, '.', ',') }} </p>
                <hr class="m-0">
